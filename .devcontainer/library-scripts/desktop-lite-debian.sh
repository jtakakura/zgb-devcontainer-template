...
fluxbox_menu="$(cat \
<< 'EOF'
[begin] ( Gameboy Development )
    [exec] (Terminal) {xterm -bg black -fg white -e "cd workspace && /bin/bash"}
    [exec] (Gameboy Tile Designer) {wine /opt/gbtd/GBTD.EXE}
    [exec] (Gameboy Map Builder) {wine /opt/gbmb/GBMB.EXE}
    [exec] (Gameboy Emulator - Emulicious) {java -jar -Dsun.java2d.xrender=false /opt/emulicious/Emulicious.jar}
    [exec] (Gameboy Emulator - BGB) {wine /opt/bgb/bgb.exe}
